...
import type { User, Group, Message } from "@prisma/client";

interface ConversationAreaProps {
  conversationId: string;
  entity: User | Group;
  user: User; // 修正: userを追加
}
    ...
    <div className="flex h-full flex-col">
      <div className="border-b p-4">
        <h2 className="text-lg font-semibold">
          {("name" in entity ? entity.name : "") ?? "Chat"}{" "}
          {/* グループ名またはユーザー名を表示 */}
        </h2>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 w-full border-t bg-white p-4 dark:bg-black">
        <form
          onSubmit={(e) => {
            e.preventDefault();
